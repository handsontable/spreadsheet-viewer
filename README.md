...
- `yarn start` - start a development server (`webpack-dev-server`) for the `sv` project on port `5000` (customizable with `--port`, e.g. `yarn start --port 8080`), rebuilding the bundle on any file changes. Then open [http:/localhost:5000](http:/localhost:5000). This is most desired for usual development, see examples below.
...
```
You can also build both bundles to serve the two bundles simultaneously:


### Testing with snapshots

It's common to have a need to test with frequently. `webpack-dev-server` doesn't support being ran in cypress, we need to therefore use a separate build and server.

To keep the tests quick, it's ideal to build a `modern` bundle for production with `MINIMIZE` set to `false`, and serve it with `yarn serve:sv`, like so:

```bash
BUNDLE=modern MINIMIZE=false yarn build:sv && yarn serve:sv # This should take ~8 seconds
# Then in another terminal window
yarn test:snapshot
```

### Running cross-browser snapshot tests
...
```bash
MINIMIZE=false yarn build:sv && yarn serve:sv # (omit BUNDLE=modern because we will also run IE11 tests)
# ...or simply
yarn start
```
...
Before you run the specs, you need to do pre-steps:
  1. `yarn build`
  2. `yarn serve:sv`
...
This repo contains a custom performance measurement tool that is used in the CI workflow and can also be used manually. It is explained on the page https://github.com/handsontable/spreadsheet-viewer-dev/wiki/Performance.
